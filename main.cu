...
__global__
void sgd_kernel(float* X, float* y, float *w, unsigned long long int num_samples 
                ,unsigned long long int dimensionality, unsigned long long int H
                ,int num_iterations);
...
int main()
{
	...
	cudaStatus = cudaMemcpy(w, host_w, nThread * dimensionality * sizeof(float), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
    ...
    cudaStatus = cudaMemcpy(host_w, w, nThread * dimensionality * sizeof(float), cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
                ...
                ,int num_iterations)
{
    ...
    /*for (unsigned long long int i=startIndex; i<endIndex; i++)
    {
        if(i < num_samples)
        {
            for (unsigned long long int j=0; j<dimensionality; j++)
            {
                X[index(i,j,dimensionality)] += 1;
            }
        }
    }*/
}
                        ...
                        ,unsigned long long int num_samples )
{
    ...
    for(unsigned long long int j=0; j<dimensionality; j++)
    {
        w[index(threadId, j, dimensionality)] -= stepSize *(xwsum * X[index(t, j, dimensionality)]);
    }
}
